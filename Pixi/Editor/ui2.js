...
function onPaintStart(event) {
	if (this.onmymouse){
		...
		if (shift.isDown){
			...
			//this.myclone.position.x = (Math.round(Math.ceil(newPosition.x - this.myx) / tile_width) * tile_width) - tile_width/2.0
			this.myclone.position.x = Math.round((newPosition.x / tile_width) * tile_width) - tile_width/2.0
			//this.myclone.position.y = (Math.round(Math.ceil(newPosition.y - this.myy) / tile_height) * tile_height) - tile_height/2.0
			this.myclone.position.y = Math.round((newPosition.y / tile_height) * tile_height) - tile_height/2.0
			this.myclone.alpha = 0.5
			this.onmymouse = true
			world.addChild(this.myclone)
			this.myclone
				.on('mousedown', onPaintStart)
				.on('mousemove', onPaintMove)
...
for (var i = 0; i < shipBlocks.length; i++){
	...
	var tilename = new PIXI.Text(shipBlocks[i].name,{font: '35px', fill: 'white', align: 'left'});
